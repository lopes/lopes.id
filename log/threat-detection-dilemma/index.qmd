...
As a CSIRT Engineer, I raised many concerns and criticisms about low-precision analytics. Now, on the Threat Detection team, I focus on writing rules that are both useful and precise, to cut the alert fatigue that stresses CSIRT. But there's a catch: overly precise rules can blind us to real malicious activity. I call this the "Threat Detection Fundamental Dilemma," and it's what this post is about.

## Reviewing Terminology

Before we get into the dilemma, let's agree on some terminology for alert investigation. Any alert CSIRT handles fits into a [confusion matrix](https://en.wikipedia.org/wiki/Confusion_matrix), with these categories:
...
The focus here is on TP and FN, which involve relevant events. Keep these in mind. With this data, we can calculate two key metrics: **Precision** and **Recall**.

::: {.callout-tip}
Keeping CSIRT's confidence high matters, so the alert rationale should stay as simple as possible. Incident Responders must tag alerts as TP or FP after triage and if additional metrics are desired, subcategories like Malware and Phishing for TP, and Anomaly and Internal Testing for FP can be created. However, simplicity (KISS) is key to success based on my experience.
:::
...
In contrast to Precision, which considers irrelevant events (FP), Recall encompasses all relevant events (TP and FN). As it accounts for undetected malicious events (FN), it is harder to calculate. This data can come from several sources: manual reports, Purple Team exercises, threat hunts, etc. Once FN per rule is obtained, Recall can be calculated using this formula:

$$
  Recall = \frac{TP}{TP + FN}
$$

Higher Recall is better: it means we caught more of the malicious events. So rules with a high Recall rate are what we want.

## Navigating by Metrics

Precision and Recall are complementary, each monitoring a distinct aspect of the scenario. As previously mentioned, Precision gauges the stress induced in operations due to FPs, while Recall evaluates the risk arising from the failure to detect malicious activities. Consider the following image, based on [this example](https://en.wikipedia.org/wiki/File:Precisionrecall.svg) from Wikipedia.

...
- Prior cases involving the same entity are retrievable cheaply, and were handled well enough to be worth learning from.
- The runbook for rule `DET-0142` is retrievable by that ID, not by searching a wiki.

Don't read that as a baseline. It's a target, and a demanding one: I've never seen a team answer yes to all six. Asset criticality at alert time is the hardest, since it usually means a CMDB that is accurate on the day you query it, and I have never seen one stay accurate for long. Retrieval is the other trap: if prior cases were handled badly, cheap retrieval just hands the agent confident precedent for a bad decision, which is worse than no precedent at all.

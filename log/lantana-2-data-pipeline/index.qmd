...
date: 2026-06-09
description: "Building Lantana's data pipeline with Claude and first end-to-end deployment."
categories: ["engineering", "detection", "deception"]
image: "og-lantana-2-data-pipeline.webp"
---

In the [previous post](link_here), I talked about Lantana's motivation, early development, technical stack, and the Alpha version delivery. After that, I took a strategic pause to learn more about AI and understand what had changed during that period to justify the buzz I was seeing in my network. After around a month playing with **Claude Opus 4.6** and **Claude Code**, I could see the hype was real: AI was more helpful and precise than ever. Inevitably, I started envisioning how helpful it would be for Lantana, and by late April, [I onboarded Claude to Lantana](https://github.com/lopes/lantana/commit/f6f69f54e33932bae3dc7592d65073c9c868deec).

## Full Review

The very first thing I did was ask Claude to review the codebase to spot and fix errors and inconsistencies. It found many low- and medium-severity issues that required changes, especially in shell scripts and configuration files. The programming skills of Claude Code are unbeatable.
...
Right after reviewing and improving the codebase, Opus started what I was calling "Phase 2": the data pipeline implementation. After a few rounds of back-and-forth with Claude to plan the tasks (including letting it interview me) it gave the green light to start implementing. I instructed it to follow Test-Driven Development (TDD) and moved to the passenger seat to watch it work. I asked it to create a phased plan, grouping tasks by functionality, and it was a good call. I was aware I had to keep the context window under control, and with so many tasks it would inevitably fill at some point. We'd need to pause and restart in a new session — I prefer that over simply compacting the current context.
...
After learning the new AI capabilities, I realized my new role was closer to that of an orchestrator, a peer reviewer, a guide to Claude. All development would be done by my new collaborator. For a few days, my life became monochromatic but eventually I saw the upside and opportunities the new era was offering, and decided to fully embrace it. I started leaving the driver's seat to Claude.
:::
...
All of this happened in roughly one week: onboard Claude, review the codebase, implement and test the pipeline. I declared the version stable enough to move to the VPS and did so on May 23rd — the Beta version, the first end-to-end version deployed to the VPS, from server setup to dashboard generation.

## A Working Pipeline

While the Beta version was running, I started reviewing my notes and analyzing reports to tune minor details. Having worked on a robust SIEM, I know that not all IOCs are equal, so I reconsidered the TTLs used in cached IOCs for third-party enrichment. I defined that benign IOCs (not flagged as malicious by the platforms) would expire in one week. Malicious ones would vary by type: 28 days for IP addresses and domains, and 56 days for hashes. The underlying logic: if something is benign, it can become malicious faster than something malicious can become benign.
...
Then, every day after midnight, the ETL ran, rotating raw logs and starting the normalization and enrichment routines that produced the Silver layer — stored on disk as Parquet. The pipeline normalizes Cowrie's field names to an OCSF-inspired schema, enriches against AbuseIPDB and VirusTotal (daily batch, HTTP APIs), and redacts the honeypot's destination IP (`dst_endpoint_ip: null`) to prevent WAN IP leakage into reports. One row per event; null columns exist for event types that don't apply (e.g., `command` and `file_*` are null for a login event).
...
Since the dashboard requires SSH tunneling to the server, I wanted a lighter version of this data I could check asynchronously. I asked Claude to generate a Markdown file as a simplified dashboard. This report is shared via Discord daily, along with the pipeline execution summary.
...
::: {.callout-warning}
Another minor issue with Dionaea was the Docker image tag. With Cowrie, I had been using `latest` successfully and started the same way with Dionaea. However, Dionaea developers have somewhat neglected this tag. Despite frequent code updates, `latest` dates from several years ago. I switched to the `nightly` tag. It feels a bit unstable, but it's workable.
:::
...
I see Claude as a tool I'd like to always have with me, to work faster, better, and more efficiently — not as a threat.

## What's Next

I now have a stable version of Lantana running in production for a few weeks. In the next and final post, I'll come back with the end of validation phase (Beta) and the version 1.0.0 in production. I'll also bring an analysis of what I learned here and what Lantana is actually delivering, wrapping up this amazing project.

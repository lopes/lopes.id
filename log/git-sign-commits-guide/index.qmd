...
Now, instruct Git to sign the commits using the `-S` flag along with the `commit` command—for example: `git commit -S -m "signed commit"`. If everything is fine, your commit should be signed, and you can check it with:

```sh
git log --show-signature
```

When you push it to GitHub, it'll check the signed commit data and evaluate if the signing key is trusted. If it is, you'll see a green "verified" tag, but if not, it'll show a yellow "unverified" tag, indicating that despite the signature, the key is not recognized. Note that in the latter case, the signature is valid but not trusted.

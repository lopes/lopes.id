...
With both CSV files in hand, run `derive-seeds.py` (source in [References](#references)) to generate a monitoring plan:
...
At this point, the assessment is done and all necessary information is collected. It's time to deploy.
...
::: {.callout-note}
You might think: *"why not train the model on labeled data instead?"* As a security engineers coming from detection engineering, I reach for this — a hand-curated training set with clean, labeled spikes and dips should produce a more accurate model, right?
...
If the validation is OK at this point, deployment went fine. From here it's monitoring and tuning for the next days: adjusting thresholds, adding or suppressing sources where needed. Have the same discipline as during Assessment: **investigate first, tune second**. An ARIMA alert is a real statistical anomaly — reach for understanding the source's behavior before you reach for a higher threshold to silence it.

See my example. After moving it to production, I noticed a lot of alerts from GITHUB, all spikes. However, GitHub is one of my oldest log sources. It's been running at the company for ages. ARIMA+ should be aware of that. It was only when I sat with the problem that it clicked: due to the AI hype and the constant increase of people using it (plus more and more agents running in the environment) the use of GitHub had greatly increased. It made ARIMA+'s predictions wrong because the baseline had shifted. With this information, I had to decide either to accept that spike alert or bump the ARIMA+ threshold to suppress it, accepting that it would make the predictions less accurate. At the end of the day, as with any detection logic, there's no right or wrong. It's just what better fits your scenario.

::: {.callout-note}
I run this in a distributed, mixed-workforce setup with fast source churn, heavy 24/7 automation, and human-driven volume growing steadily — AI adoption on GitHub, for example.
...
A useful query for tracking ARIMA+ alert quality over time. Pay attention to `per_day > 2`: that's a signal to bump the `anomaly_prob_threshold` one step — from 0.99 to 0.995, then 0.998 or 0.999. `on_weekend / (on_weekend + on_weekday) > 50%` is the same signal from a different angle — since BQML often misses `WEEKLY` seasonality (see Deployment), weekend/weekday imbalance is a leading indicator that a source needs a higher threshold. Compare against the source's `wd_we_ratio` in `signals.csv`: if that ratio was well above 1.0 and weekend alerts dominate here, the model isn't compensating for the pattern the raw data has.

...
3. ⬆️ **Spike:** a source sends significantly more data than expected
4. ⬇️ **Dip:** a source sends significantly less data than expected, but hasn't stopped entirely
...
::: {.callout-note}
The hourly detection query scans `ingestion_metrics` for the `FULL_STOP` heartbeat check, and to keep that scan cheap it only looks back 14 days — 336h. Any source silent beyond that returns `latest_event = NULL` and fires `FULL_STOP` regardless of `max_silence_hours`; the scan window short-circuits the threshold check. When that happens the alert reports the silence as a floor — `≥336h` — rather than a precise figure, since the exact duration is unknowable inside the window (stamping a hardcoded sentinel instead would just fabricate a number). Scanning further back is possible but scales cost linearly, and 14 days is the pragmatic tradeoff. Sources that legitimately need >336h tolerance belong in `suppressions` (temporary) or `excluded_log_types` (permanent) instead. Both cost nothing to evaluate.
:::
...
CREATE OR REPLACE TABLE `chronicle-self.log_health.anomalies` (
  ...
  -- FULL_STOP: hours_silent vs. max_silence_hours, value_unit='h' (or 'h+' when the
  --   feed has aged out of the 336h lookback, so the value is a floor: >= the window).
...
-- When a feed ages out of the 336h lookback, latest_event is NULL and its exact
-- silence is unknowable, so actual/delta are reported as a floor (>= the window)
-- and value_unit is stamped 'h+' — never a fabricated sentinel. The 336 literal
-- MUST match latest_per_source's INTERVAL 336 HOUR below.
INSERT INTO `chronicle-self.log_health.anomalies`
...
candidates AS (
  SELECT c.source_key, c.max_silence_hours, l.latest_event,
         TIMESTAMP_DIFF(CURRENT_TIMESTAMP(), l.latest_event, HOUR) AS hours_silent
  FROM `chronicle-self.log_health.source_config` c
  LEFT JOIN latest_per_source l ON l.source_key = c.source_key
  WHERE c.enabled
)
SELECT CURRENT_TIMESTAMP(), source_key, 'FULL_STOP', latest_event,
       CAST(COALESCE(hours_silent, 336) AS FLOAT64)                     AS actual_value,
       CAST(max_silence_hours AS FLOAT64)                               AS expected_value,
       CAST(COALESCE(hours_silent, 336) - max_silence_hours AS FLOAT64) AS delta_value,
       ROUND(SAFE_DIVIDE(COALESCE(hours_silent, 336) - max_silence_hours,
                         max_silence_hours) * 100, 1)                   AS severity_pct,
       IF(hours_silent IS NULL, 'h+', 'h')                             AS value_unit
FROM candidates
WHERE (latest_event IS NULL OR hours_silent > max_silence_hours)
  AND NOT EXISTS (
    SELECT 1 FROM `chronicle-self.log_health.suppressions` s
    WHERE s.source_key = candidates.source_key
      AND (s.until IS NULL OR s.until > CURRENT_TIMESTAMP())
  )
  AND NOT EXISTS (
    SELECT 1 FROM `chronicle-self.log_health.anomalies` x
    WHERE x.source_key = candidates.source_key
...
- [`log-health.gs`](https://gist.github.com/lopes/041a25c7792303eb15ab600251f5c11b)
